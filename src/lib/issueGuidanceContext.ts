...
export function getStaticGuidance(
  ...
  if (isServiceable) {
    return {
      ...
      recommendation: `We recommend addressing this issue to protect your current unit. The cost of fixing this now is much less than the potential damage from a failure.`,
      economicContext: `Potential damage if failure occurs: $${damageScenario.waterDamage.min.toLocaleString()}-$${damageScenario.waterDamage.max.toLocaleString()}. Fixing this issue costs $${issue.costMin}-$${issue.costMax}.`,
      actionItems: [
        'Schedule a service appointment to address this issue',
        'Ask about bundling with annual maintenance for savings',
    ...
    return {
      ...
      economicContext: `Your replacement quote will include ${issue.friendlyName.toLowerCase()} as part of the complete installation package.`,
      actionItems: [
        'Get a replacement quote that includes proper infrastructure',
        'Ask about financing options for a complete system upgrade',
        'Learn what protections are included in each installation tier',
      ],
      shouldFix: false,
    };
  }
